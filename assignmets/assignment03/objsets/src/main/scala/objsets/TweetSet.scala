...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def union(that: TweetSet): TweetSet = {
    new NonEmpty(this.elem, 
        left.union(that.filter(
            (p: Tweet) => {p.text < this.elem.text})),
        right.union(that.filter(
            (p: Tweet) => {p.text > this.elem.text}))
        )
  }
  ...
  def mostRetweetedAcc(max:Tweet): Tweet = {
    if (max.retweets < elem.retweets) right.mostRetweetedAcc(left.mostRetweetedAcc(elem))
    else right.mostRetweetedAcc(left.mostRetweetedAcc(max))
  }
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter(
		  														(t: Tweet) => {
		  														  google.exists((text: String) => (t.text.contains(text)))
		  														})
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter(
		  														(t: Tweet) => {
		  														  apple.exists((text: String) => (t.text.contains(text)))
		  														})
